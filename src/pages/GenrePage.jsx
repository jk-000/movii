...
const GenrePage = () => {
  ...
  const handlePageChange = (page) => {
    setCurrentPage(page);
    navigate(`/genre/${genre}?page=${page}`);
  };
  ...
  const {
    ...
    queryKey: ["genreMovies", genre, getPageFromUrl()],
    queryFn: () =>
      fetchMoviesByGenre(genre.toLowerCase(), getPageFromUrl(), limit),
  ...
  // SEO: Set title and meta tags dynamically
  useEffect(() => {
    const capitalized =
    ...
    const descriptionContent = `Download latest ${capitalized} movies and web series in HD on JKHub Movies. Free streaming & download.`;
    ...
    const keywords = `JKHub Movies, ${capitalized} Movies, Free ${capitalized} Web Series Download, Watch ${capitalized} Movies Online`;
